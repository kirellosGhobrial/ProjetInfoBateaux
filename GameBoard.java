...
public class GameBoard extends Frame {
	...
	public GameBoard(){
		...
		//top
		top.setSize(100,50);
		top.add(lblTop, BorderLayout.WEST);
			...
			if(allowMouse1){
				...
				if(batDir == 1){
					for(int i = coor[0]; i<coor[0]+batLen; i++){
						if(i>=area1.length){
							lbl1.setText( coor[0] + " et " + coor[1] + "pas d'espace");
							color = Color.RED;
							break;
						}else if(area1Int[i][coor[1]] == 1){
							lbl1.setText( coor[0] + " et " + coor[1] + " il y a un bateau ");						
							color = Color.RED;
							break;
						}
					}
		...
		for(int i=0; i<tab.length;i++){
			for(int j=0; j<tab[0].length; j++){
				if(tab[i][j] == 0){
					...
					area1[i][j].setBackground(Color.GREEN);
				}else{
	...
	public void placerBateau(Joueur joueur){
		switch(nbBat){
			case 0: 
				lbl2.setText("placer le premier bateau, c'est un porte-avion de taille 5");
				batLen = 5;
				break;
			case 1:
				Bateau bat1 = new Bateau(1, batDir, coor[1], coor[0]);
				joueur.ajouteBateau(bat1);
				lbl2.setText("placer le deuxième bateau, c'est un cargo de taille 4");
				batLen = 4;
				break;
			case 2:
				Bateau bat2 = new Bateau(2, batDir, coor[1], coor[0]);
				joueur.ajouteBateau(bat2);
				lbl2.setText("placer le deuxième bateau, c'est un voilier de taille 3");
				batLen = 3;
				break;
			case 3:
			...
			case 4:
				...
				lbl2.setText("placer le deuxième bateau, c'est un barque de taille 2");
				batLen = 2;
				break;
			case 5:
